...
;; (defun contrast-color (color &optional hc)
;;   (color-lighten-name
;;    (color-blend-name
;;     (color-saturate-name color saturation-amount)
;;     (if hc hc-blend-with lc-blend-with) blend-amount)
;;    (if hc lighten-amount darken-amount) ))

(defun contrast-color (color &optional hc)
  (let* ((c color)
         (lc (not hc))
         (light (eq variant 'light))
         (dark (not light))
         ;; (bl-a 0.9)
         (bl-a 0.8)
         (bri-a (cond ((and light hc) 12)
                      ((and light lc) -12)
                      ((and dark hc) 12)
                      ((and dark lc) -12)))
         (saturation-amount 20 )
         ;; (hc-blend-with solarized-emph)
         (hc-blend-with solarized-emph)
         (lc-blend-with solarized-hl)
         (bl-c (if hc hc-blend-with lc-blend-with) )

         (c (color-saturate-name c saturation-amount))
         (c (color-lighten-name c bri-a))
         (c (color-blend-name c bl-c bl-a))
         )
    c
    )
)
...
customize the resulting theme."
  (let* (
         ...
         (yellow-lc  (contrast-color yellow))
         (yellow-hc  (contrast-color yellow t))
         (orange-lc  (contrast-color orange))
         (orange-hc  (contrast-color orange t))
         (red-lc  (contrast-color red))
         (red-hc  (contrast-color red t))
         (magenta-lc  (contrast-color magenta))
         (magenta-hc  (contrast-color magenta t))
         (blue-lc  (contrast-color blue))
         (blue-hc  (contrast-color blue t))
         (cyan-lc  (contrast-color cyan))
         (cyan-hc  (contrast-color cyan t))
         (green-lc  (contrast-color green))
         (green-hc  (contrast-color green t))
